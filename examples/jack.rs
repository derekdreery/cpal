//! Similar to the `beep` example, but configured to use `jack`.
...
fn main() -> Result<(), anyhow::Error> {
    ...
    let mut host = cpal::platform::JackHost::new().expect("jack host unavailable");
    host.set_connect_automatically(false);
    let host = cpal::Host::from(host);
    let device = host
